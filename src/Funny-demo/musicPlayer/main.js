(function() {
  ...
  // 读取到的音频文件时2进制文件（可以使用Ajax远程加载，可以使用FileAPI读取本地文件），我们需要让AudioContext先对其解码，然后再进行后续擦做。
  ...
  gainnode.gain.value = 1;//改变gainnode.gain.value来实现静音
    ...
    bind: function() {
      ...
      $('.music-file').on('change', function() {
        ...
        files.forEach(function(file) {
          ...
          fr.readAsArrayBuffer(file);//按字节读取文件内容，结果用ArrayBuffer对象表示
          var mdata = {
          ...
          fr.onload = function(e) {//加载成功onload
  ...
  Retangle.prototype.update = function (power) {
    this.power = power;
  ...
  function animate() {
    if (!musics[nowIndex].decoding) {
      ...
      analyser.getByteFrequencyData(array);	//将音频节点的数据拷贝到Uint8Array中
      ...
      for (var i = 0; i < rt_array.length; i++) {
        ...
        rt.index = ('index' in rt) ? rt.index : ~~(rt.x * bili);//~~运算符将变量转化为Number类型
        console.log(rt , ~~(rt.x * bili));
